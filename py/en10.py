...
# ==============================================================================
# XuePilot OER Spider (V16.0 现代前沿与太空纪元版 - 突破古典局限)
# 核心使命：在经典文学基础上，注入 Simple English 现代科技与宇宙探索海量语料！
# ⚠️ 警告：全站遍历极度耗时，请挂机运行！
# ==============================================================================
...
class XuePilotK12Spider:
    ...
    # ==========================================================================
    # 🚀 集群 8：全球传世儿童文学大满贯 (Global Children's Literature Classics)
    # ==========================================================================
    def harvest_global_children_literature(self):
        print("\n" + "★"*60)
        print(" 🚀 [集群 8: 儿童文学大满贯] 启动全球 10 大最伟大儿童名著全量收割...")
        print("★"*60)
        
        books = [
            ("tb_lit_alice_wonderland", "Alice's Adventures in Wonderland (爱丽丝梦游仙境)", "https://www.gutenberg.org/files/11/11-h/11-h.htm", "🐇"),
            ("tb_lit_looking_glass", "Through the Looking-Glass (爱丽丝镜中奇遇记)", "https://www.gutenberg.org/files/12/12-h/12-h.htm", "🪞"),
            ("tb_lit_wizard_oz", "The Wonderful Wizard of Oz (绿野仙踪)", "https://www.gutenberg.org/files/55/55-h/55-h.htm", "🌪️"),
            ("tb_lit_secret_garden", "The Secret Garden (秘密花园)", "https://www.gutenberg.org/files/113/113-h/113-h.htm", "🌿"),
            ("tb_lit_black_beauty", "Black Beauty (黑骏马)", "https://www.gutenberg.org/files/271/271-h/271-h.htm", "🐴"),
            ("tb_lit_wind_willows", "The Wind in the Willows (柳林风声)", "https://www.gutenberg.org/files/289/289-h/289-h.htm", "🐸"),
            ("tb_lit_peter_pan", "Peter Pan (彼得·潘)", "https://www.gutenberg.org/files/16/16-h/16-h.htm", "🧚"),
            ("tb_lit_treasure_island", "Treasure Island (金银岛)", "https://www.gutenberg.org/files/120/120-h/120-h.htm", "🏴‍☠️"),
            ("tb_lit_tom_sawyer", "The Adventures of Tom Sawyer (汤姆·索亚历险记)", "https://www.gutenberg.org/files/74/74-h/74-h.htm", "👦"),
            ("tb_lit_heidi", "Heidi (海蒂)", "https://www.gutenberg.org/files/1448/1448-h/1448-h.htm", "⛰️")
        ]
        ...
        for book_id, title, url, icon in books:
            ...
            try:
                ...
                print(f"    ✅ 成功解析 {len(chapters)} 个文学章节。")
                data = { 
                    "code_id": book_id, "title": title, "provider_type": "Children's Literature (K-8)", 
                    "icon": icon, "is_free": True, "source_url": url, 
                    "description": "英语世界最伟大的儿童文学长篇巨著。绝佳的想象力、共情力与高阶词汇启蒙底座，适合深度伴读与探究。", 
    ...
    # ==========================================================================
    # 🚀 集群 9：现代前沿科技与世界认知 (Simple English Wikipedia - 现代儿童百科)
    # ==========================================================================
    def harvest_modern_frontier_knowledge(self):
        print("\n" + "★"*60)
        print(" 🚀 [集群 9: 现代前沿与新世界] 启动 Simple English 现代科技与探索全量收割...")
        print("★"*60)
        
        # 使用专为英语初学者和儿童(K-6/ESL)打造的 Simple English Wikipedia
        # 涵盖最前沿的、十九世纪名著里绝对没有的现代知识！
        topics = [
            ("tb_mod_artificial_intelligence", "Artificial Intelligence (人工智能)", "Artificial_intelligence", "🤖"),
            ("tb_mod_space_exploration", "Space Exploration (太空探索)", "Space_exploration", "🚀"),
            ("tb_mod_mars_rover", "Mars Rover (火星漫游车)", "Mars_rover", "🛸"),
            ("tb_mod_internet", "The Internet (互联网)", "Internet", "🌐"),
            ("tb_mod_climate_change", "Climate Change (气候变化)", "Climate_change", "🌍"),
            ("tb_mod_virtual_reality", "Virtual Reality (虚拟现实)", "Virtual_reality", "🥽"),
            ("tb_mod_renewable_energy", "Renewable Energy (可再生能源)", "Renewable_energy", "☀️"),
            ("tb_mod_black_hole", "Black Hole (黑洞)", "Black_hole", "🌌"),
            ("tb_mod_quantum_mechanics", "Quantum Mechanics (量子力学基础)", "Quantum_mechanics", "⚛️"),
            ("tb_mod_dinosaur_extinction", "Cretaceous–Paleogene extinction event (恐龙灭绝之谜)", "Cretaceous%E2%80%93Paleogene_extinction_event", "☄️")
        ]

        for topic_id, title, slug, icon in topics:
            if self._check_exists(topic_id): continue
            print(f"\n  ⏳ 正在链接维基数据中心，抓取现代百科: 《{title}》...")
            ...
            try:
                ...
                # 清洗无用的引用和导航模块
                for el in content_div.find_all(['table', 'div', 'span'], class_=['infobox', 'navbox', 'reflist', 'mw-editsection', 'reference']):
                    el.decompose()

                # 将整篇维基文章按照 h2 拆分为不同的章节 (Chapters)
                chapters = []
                current_chapter_title = "Introduction / 综述"
                current_content = []
                ...
                for element in content_div.children:
                    if element.name == 'h2':
                        ...
                        current_chapter_title = element.get_text(strip=True)
                        current_content = []
                    elif element.name in ['h3', 'p', 'ul']:
                        if element.name == 'p':
                            text = element.get_text(strip=True)
                            # 正则表达式清除残留的维基百科脚注，例如 [1], [2]
                            text = re.sub(r'\[\d+\]', '', text)
                            if text:
                                current_content.append(f"<p class='mb-6 text-slate-300 leading-loose text-lg tracking-wide'>{text}</p>")
                        elif element.name == 'h3':
                            text = element.get_text(strip=True)
                            current_content.append(f"<h3 class='text-2xl font-bold text-cyan-300 mt-8 mb-4'>{text}</h3>")
                        elif element.name == 'ul':
                            items = "".join([f"<li class='ml-6 list-disc mb-2 text-slate-300'>{li.get_text(strip=True)}</li>" for li in element.find_all('li')])
                            if items:
                                current_content.append(f"<ul class='mb-6'>{items}</ul>")

                if current_content:
                    chapters.append({"title": current_chapter_title, "content": "".join(current_content)})

                print(f"    ✅ 成功解析 {len(chapters)} 个前沿知识板块。")
                data = { 
                    "code_id": topic_id, "title": title, "provider_type": "Modern Frontier (Simple Wiki)", 
                    "icon": icon, "is_free": True, "source_url": url, 
                    "description": "专为儿童与第二语言学习者打造的 Simple English 现代前沿百科。用最基础的词汇解释人工智能、量子力学、气候变化等当代最尖端的命题。", 
                    "chapters_json": chapters 
                }
                self.push_to_db("edu_textbooks", data)
                time.sleep(1.5)
...
if __name__ == "__main__":
    print("="*75)
    print("   XuePilot 航母级全球教研爬虫矩阵 (V16.0 现代前沿与太空纪元版)   ")
    print("   ⚠️ 本次扫描已接入宇宙探索、AI与现代前沿科技！  ")
    ...
    # 8. 全球传世儿童文学大满贯 (10部巅峰长篇巨著)
    spider.harvest_global_children_literature()
    ...
    # 🔥 9. 现代前沿与太空纪元 (Simple English 现代科技)
    spider.harvest_modern_frontier_knowledge()
    
    print("="*75)
    print(" 🏆 V16.0 现代前沿版：全宇宙资源收割完毕！")
    print(" 您的数据库不仅有莎士比亚，现在还有 SpaceX、人工智能和量子力学！")
    print("="*75)